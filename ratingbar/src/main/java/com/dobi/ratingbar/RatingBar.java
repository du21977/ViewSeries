...
// 移动 按下 抬起 处理逻辑都是一样，判断手指的位置，根据当前位置计算出分数，再去刷新显示
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: // 移动
                // case MotionEvent.ACTION_UP: // 抬起 尽量减少onDraw()的调用 up事件和down事件对应，同一个事件，可以不要
                float moveX = event.getX();//event.getX()相对于当前控件自己的位置   event.getRawX()获取幕的x位置
