...
public class LetterSideBar extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                // 计算出当前触摸字母  event.getY()获取在当前控件的位置   event.getRawX()获取当前位置在屏幕的位置
                float currentMoveY = event.getY();
                // 位置 = currentMoveY / 字母高度 ， 通过位置获取字母  优化？
                int itemHeight = (getHeight() - getPaddingTop() - getPaddingBottom()) / mLetters.length;  //每个字母的高度
    ...
    // 接口回掉其他View会不会使用？，不会就写在这个类，会就单独写一个接口文件
